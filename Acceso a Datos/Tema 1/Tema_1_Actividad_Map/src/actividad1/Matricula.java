...
public class Matricula {
	...
	public Matricula(String matricula) throws MatriculaException {
		if (matricula==null)
			throw new MatriculaException(MatriculaException.ERROR_MATRICULA_NULA);
	...
	private static void compruebaMatricula(String matricula) 
		...
		if (matricula.length()!=7)
			throw new MatriculaException(MatriculaException.ERROR_LONGITUD);
		// 4 digitos – 3 letras
		// compruebo dígitos
		for(int i = 0; i<4; i++) {
			...
			if (!Character.isDigit(c)) {
				throw new MatriculaException(MatriculaException.ERROR_DIGITOS);
			}
		}
		for(int i=4; i<7; i++) {
			char c = matricula.charAt(i);
